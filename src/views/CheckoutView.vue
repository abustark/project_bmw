<template>
  <div class="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    <div v-if="success" class="mt-6 card p-10 text-center">
      ...
      <p class="mt-2 text-zinc-600">We’ve sent your receipt and download links to <b>{{ form.email }}</b>. Order #NG-{{ orderId }}</p>
      ...
      <div class="mt-6 text-xs text-zinc-500">Digital items are available instantly. Physical items ship in 3–5 days.</div>
    </div>
    ...
    <div v-else class="mt-6 grid lg:grid-cols-12 gap-6">
      <form @submit.prevent="placeOrder" class="lg:col-span-7 space-y-4">
        <div class="card p-6">
          ...
          <p class="mt-2 text-xs text-zinc-500">We’ll auto-create your N-GELO account and email your login.</p>
        </div>

        <div class="card p-6">
          <h3 class="font-semibold">Payment</h3>
          <div class="mt-3 flex gap-2">
            <button type="button" @click="method='card'" :class="['flex-1 py-3 rounded-xl border font-medium text-sm', method==='card' ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white']">Card (Stripe)</button>
            <button type="button" @click="method='paypal'" :class="['flex-1 py-3 rounded-xl border font-medium text-sm', method==='paypal' ? 'bg-zinc-900 text-white border-zinc-900' : 'bg-white']">PayPal</button>
          </div>
        ...
        <button type="submit" :disabled="!cart.items.length" class="w-full btn-primary py-4 text-base">Pay ${{ cart.total }} — Complete order</button>
        <p class="text-xs text-center text-zinc-500">Secure 256-bit SSL • Instant download after payment</p>
      </form>
        ...
        <div class="card p-6 sticky top-[80px]">
          ...
          <div class="mt-4 space-y-3">
            <div v-for="i in cart.items" :key="i.id" class="flex gap-3">
              ...
              <div class="flex-1"><div class="text-sm font-medium leading-tight">{{ i.name }}</div><div class="text-xs text-zinc-500">Qty {{ i.qty }} • ${{ i.price }}</div></div>
              <div class="text-sm font-semibold">${{ i.price * i.qty }}</div>
            </div>
          </div>
          <div class="mt-4 border-t pt-4 space-y-2 text-sm">
            ...
            <div v-if="cart.discount" class="flex justify-between text-green-600"><span>Discount</span><span>−${{ cart.discount }}</span></div>
            <div class="flex justify-between font-bold text-lg"><span>Total</span><span>${{ cart.total }}</span></div>
          </div>
          <div class="mt-4 p-3 rounded-xl bg-zinc-50 border text-xs text-zinc-600">Digital delivery is instant. Physical footwear ships tracked — you’ll get a DHL link.</div>
        </div>
      </div>
    </div>
  </div>

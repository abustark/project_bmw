...
export default function CartPage() {
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <h1 className="font-bold text-3xl" style={{fontFamily: "var(--font-playfair)"}}>Cart</h1>
      <p className="text-sm text-zinc-500">{cart.items.reduce((n,i)=>n+i.qty,0)} items</p>
      ...
      {cart.items.length===0 ? (
        ...
        <div className="mt-6 grid lg:grid-cols-12 gap-6">
          <div className="lg:col-span-8 space-y-3">
            {cart.items.map((item) => (
              <Card key={item.id} className="p-4 flex gap-4 rounded-[20px]">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={item.image} className="w-24 h-24 rounded-xl object-cover border bg-zinc-50" alt="" />
                <div className="flex-1">
                  <Link href={`/product/${item.id}`} className="font-medium hover:underline">{item.name}</Link>
                  <div className="text-xs text-zinc-500">{item.author} • {item.category}</div>
                  <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className="ml-auto font-bold">${item.price * item.qty}</span>
                  </div>
                </div>
          ...
          <div className="lg:col-span-4">
            <Card className="p-6 sticky top-[80px] rounded-[20px]">
              ...
              <div className="mt-4 space-y-2 text-sm">
                ...
                {cart.discount()>0 && <div className="flex justify-between text-green-600"><span>Discount ({cart.coupon})</span><span>−${cart.discount()}</span></div>}
                <div className="flex justify-between font-bold text-lg border-t pt-3"><span>Total</span><span>${cart.total()}</span></div>
              </div>
              ...
              <p className="mt-3 text-xs text-zinc-500 text-center">Secure checkout • Instant downloads</p>
            </Card>
          </div>
        </div>
      )}

...
export default function CheckoutPage() {
  ...
  if (success) {
    return (
      <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <Card className="mt-6 p-10 text-center rounded-[20px]">
          ...
          <p className="mt-2 text-zinc-600">We’ve sent your receipt and download links to <b>{form.email}</b>. Order #NG-{orderId}</p>
          ...
          <div className="mt-6 text-xs text-zinc-500">Digital items are available instantly. Physical items ship in 3–5 days.</div>
        </Card>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-[1280px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <h1 className="font-bold text-3xl" style={{fontFamily: "var(--font-playfair)"}}>Checkout</h1>
      <div className="mt-6 grid lg:grid-cols-12 gap-6">
        <form onSubmit={placeOrder} className="lg:col-span-7 space-y-4">
          <Card className="p-6 rounded-[20px]">
            ...
            <p className="mt-2 text-xs text-zinc-500">We’ll auto-create your N-GELO account and email your login.</p>
          </Card>
          ...
          <Button type="submit" disabled={!cart.items.length} variant="pill" size="pill" className="w-full py-6 text-base">Pay ${cart.total()} — Complete order</Button>
          <p className="text-xs text-center text-zinc-500">Secure 256-bit SSL • Instant download after payment</p>
        </form>

        <div className="lg:col-span-5">
          <Card className="p-6 sticky top-[80px] rounded-[20px]">
            ...
            <div className="mt-4 space-y-3">
              {cart.items.map((i) => (
                <div key={i.id} className="flex gap-3">
                  ...
                  <div className="flex-1"><div className="text-sm font-medium leading-tight">{i.name}</div><div className="text-xs text-zinc-500">Qty {i.qty} • ${i.price}</div></div>
                  <div className="text-sm font-semibold">${i.price * i.qty}</div>
                </div>
              ))}
            </div>
            <div className="mt-4 border-t pt-4 space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-zinc-500">Subtotal</span><span>${cart.subtotal()}</span></div>
              {cart.discount()>0 && <div className="flex justify-between text-green-600"><span>Discount</span><span>−${cart.discount()}</span></div>}
              <div className="flex justify-between font-bold text-lg"><span>Total</span><span>${cart.total()}</span></div>
            </div>
            <div className="mt-4 p-3 rounded-xl bg-zinc-50 border text-xs text-zinc-600">Digital delivery is instant. Physical footwear ships tracked — you’ll get a DHL link.</div>
          </Card>
        </div>
      </div>
    </div>
